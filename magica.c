...
static Arena *arena_new(usize size) {
    ...
    Arena *a = mem;
    *a = (Arena){.base = mem, .size = size, .used = ALIGN_UP(sizeof(Arena), 16), .committed = init};
    return a;
}
...
static bool os_commit(void *addr, usize size) { return mprotect(addr, size, PROT_READ | PROT_WRITE) == 0; }
...
#else
static void *os_reserve(usize size) { return VirtualAlloc(NULL, size, MEM_RESERVE, PAGE_NOACCESS); }
static bool os_commit(void *addr, usize size) { return VirtualAlloc(addr, size, MEM_COMMIT, PAGE_READWRITE) != NULL; }
static void os_release(void *addr, usize size) {
    (void)size;
...
static void error_at(usize pos, usize len, const char *fmt, ...) {
    ...
    fprintf(stderr, STR_FMT ":%u:%u: error: ", STR_ARG(g_err.filename), line, col);
    va_list ap;
    va_start(ap, fmt);
...
static Str8 tok_text(Token t) { return str8_range(g_err.source, t.pos, t.pos + t.len); }
static inline char lex_peek(Lexer *l, usize ahead) {
    usize i = l->pos + ahead;
    return (i < l->src.len) ? l->src.data[i] : '\0';
}
static inline char lex_advance(Lexer *l) { return (l->pos < l->src.len) ? l->src.data[l->pos++] : '\0'; }
...
static inline Token lex_make(TokenKind kind, usize start, usize end) { return (Token){.kind = kind, .pos = (u32)start, .len = (u16)(end - start)}; }
...
static TokenKind lex_keyword(Str8 s) {
    static const struct {
        ...
        {"if", 2, TK_IF},           {"in", 2, TK_IN},           {"for", 3, TK_FOR},
        {"func", 4, TK_FUNC},       {"cast", 4, TK_CAST},       {"null", 4, TK_NULL},
        {"else", 4, TK_ELSE},       {"true", 4, TK_TRUE},       {"enum", 4, TK_ENUM},
        {"false", 5, TK_FALSE},     {"break", 5, TK_BREAK},     {"match", 5, TK_MATCH},
        {"defer", 5, TK_DEFER},     {"union", 5, TK_UNION},     {"struct", 6, TK_STRUCT},
        {"return", 6, TK_RETURN},   {"sizeof", 6, TK_SIZEOF},   {"import", 6, TK_IMPORT},
        {"alignof", 7, TK_ALIGNOF}, {"foreign", 7, TK_FOREIGN}, {"continue", 8, TK_CONTINUE},
    };
...
static Token lex_next(Lexer *l) {
    ...
    if (is_alpha(c)) {
        ...
        return (Token){.kind = lex_keyword(text), .pos = (u32)start, .len = (u16)(l->pos - start)};
    }
    switch (c) {
    case '(':
...
// NODE_INVALID: unused placeholder
// NODE_TYPE_VOID: kind only (builtin type)
// NODE_TYPE_BOOL: kind only (builtin type)
// NODE_TYPE_U8: kind only (builtin type)
// NODE_TYPE_U16: kind only (builtin type)
// NODE_TYPE_U32: kind only (builtin type)
// NODE_TYPE_U64: kind only (builtin type)
// NODE_TYPE_I8: kind only (builtin type)
// NODE_TYPE_I16: kind only (builtin type)
// NODE_TYPE_I32: kind only (builtin type)
// NODE_TYPE_I64: kind only (builtin type)
// NODE_TYPE_F32: kind only (builtin type)
// NODE_TYPE_F64: kind only (builtin type)
// NODE_TYPE_PTR: ptr = element type
// NODE_TYPE_ARRAY: left = size expr (NODE_INT), ptr = element type
// NODE_TYPE_SLICE: ptr = element type
// NODE_TYPE_TUPLE: ptr = linked list of element type nodes
// NODE_TYPE_STRING: kind only (builtin type)
// NODE_TYPE_NAMED: token = name; decl = declaration node (filled by resolver)
// NODE_TYPE_GENERIC: token = base name; ptr = type argument list; decl = generic decl
// NODE_TYPE_UNTYPED_INT: literal marker (used before typechecking)
// NODE_TYPE_UNTYPED_FLOAT: literal marker (used before typechecking)
// NODE_INT: token = source text; type set by typechecker; token.len==0 used for array-size sentinel
// NODE_FLOAT: token = source text; type set by typechecker
// NODE_CHAR: token = source text; type set by typechecker
// NODE_STRING: token = source text; type = NODE_TYPE_STRING after typecheck
// NODE_BOOL: token = 'true'/'false'; type = NODE_TYPE_BOOL
// NODE_NULL: token = 'null'; type = pointer-to-void sentinel (type_null)
// NODE_IDENT: token = identifier; decl = resolved declaration (var/const/func/param/variant); type from decl
// NODE_UNDERSCORE: token = '_' placeholder; no type
// NODE_TUPLE: ptr = first element expr; next chains elements; token from first element
// NODE_ARRAY_LIT: ptr = element expr list; token holds '[' token
// NODE_STRUCT_LIT: type = type node (named/generic); ptr = list of NODE_FIELD_INIT
// NODE_FIELD_INIT: token = field name; left = initializer expr; decl set to struct field after typecheck
// NODE_UNARY: op = operator token; right = operand expr; type set by typechecker
// NODE_BINARY: op = operator token; left/right = operand exprs; type set by typechecker
// NODE_CALL: left = callee expr (often IDENT); ptr = argument expr list; type = return type after typecheck
// NODE_INDEX: left = receiver expr; right = index expr; type = element type after typecheck
// NODE_SLICE: left = receiver expr; ptr/right = optional low/high bounds; type = slice type
// NODE_FIELD: left = receiver expr; token = field name; decl = struct field node after typecheck; type = field type
// NODE_CAST: type = target type node; left = expr being cast; token = 'cast' token
// NODE_SIZEOF: left = type node; token = 'sizeof' token; type = u64
// NODE_ALIGNOF: left = type node; token = 'alignof' token; type = u64
// NODE_BLOCK: ptr = first statement; next chains statements; token = '{' token
// NODE_IF: left = condition expr; right = then expr; ptr = else expr (optional); type = branch result or void
// NODE_FOR: ptr != NULL -> iterator form (ptr is NODE_VAR iter var with left=iterable); left/right used for while-form; type = void
// NODE_MATCH: left = scrutinee expr; ptr = list of NODE_MATCH_ARM; type = common arm result
// NODE_MATCH_ARM: token = pattern token; left = pattern expr; right = result expr
// NODE_RANGE: left/right = bound exprs; op = '..' or '..='; type set to type_range marker
// NODE_RETURN: left = optional return expr; token = 'return'
// NODE_BREAK: token = 'break'
// NODE_CONTINUE: token = 'continue'
// NODE_DEFER: left = expr to defer
// NODE_IMPORT: token = string path node stored in left; used by resolver
// NODE_VAR: token = name (or first name for multi); ptr = list of name NODE_IDENTs for multi-decls; type = annotated type; left = initializer expr
// NODE_CONST: token = name; type = annotated type; left = initializer expr
// NODE_FUNC: token = name; right = type parameter list (NODE_TYPE_PARAM); ptr = parameter list (NODE_PARAM); type = return type; left = body/block
// NODE_FOREIGN: token = name; left = optional string lib node; ptr = params; type = return type
// NODE_PARAM: token = param name; type = annotated type; used in function scope
// NODE_VARARGS: marker in param list (no other fields used)
// NODE_TYPE_PARAM: token = type parameter name; used in generic declarations
// NODE_STRUCT: token = name; right = type params; ptr = NODE_STRUCT_FIELD list
// NODE_UNION: token = name; right = type params; ptr = NODE_STRUCT_FIELD list
// NODE_ENUM: token = name; ptr = NODE_ENUM_VARIANT list; enum variants get type set to named enum
// NODE_STRUCT_FIELD: token = field name; type = field type node; next chains fields
// NODE_ENUM_VARIANT: token = variant name; left = optional assigned value expr; type set to enum type
...
static Node *parse_primary(Arena *a, Parser *p) {
    ...
    if (check(p, TK_MINUS) || check(p, TK_BANG) || check(p, TK_CARET) || check(p, TK_STAR) || check(p, TK_TILDE)) {
        Token op = p->current;
        advance(p);
        Node *n = NEW(a, Node);
        n->kind = NODE_UNARY;
    ...
    for (;;) {
        ...
        if (match_tok(p, TK_DOT)) {
            if (match_tok(p, TK_LBRACE)) {
                ...
                } else if (n->kind == NODE_CALL && n->left && n->left->kind == NODE_IDENT) {
                    type_node = NEW(a, Node);
...
static Node *parse_decl(Arena *a, Parser *p) {
    ...
    if (match_tok(p, TK_COLON2)) {
        ...
        if (match_tok(p, TK_LPAREN)) {
            ...
            n->kind = NODE_FUNC;
            n->token = name;
            bool is_type_params = check(p, TK_IDENT) && (peek_next(p) == TK_COMMA || peek_next(p) == TK_RPAREN);
            if (is_type_params) {
                Node *first = NULL, *last = NULL;
                while (!check(p, TK_RPAREN) && !check(p, TK_EOF)) {
                    Node *tp = NEW(a, Node);
                ...
                expect(p, TK_RPAREN, "expected ')' after type parameters");
                n->right = first;
                expect(p, TK_LPAREN, "expected '(' for parameters after type parameters");
                n->ptr = parse_params(a, p);
                expect(p, TK_RPAREN, "expected ')' after parameters");
            } else {
                n->ptr = parse_params(a, p);
    ...
    error_at(p->current.pos, p->current.len, "expected ':', '::', or ':=' after name");
    return NULL;
}
...
static void resolve_type(Resolver *r, Node *type) {
    ...
    case NODE_TYPE_NAMED: {
        ...
        if (!decl) {
            error_at(type->token.pos, type->token.len, "unknown type '" STR_FMT "'", STR_ARG(name));
        } else {
            type->decl = decl;
        }
        break;
        ...
        if (!decl) {
            ...
            if (decl->kind == NODE_STRUCT || decl->kind == NODE_UNION) {
                ...
                for (Node *arg = type->ptr; arg; arg = arg->next)
                    got++;
                if (expected != got) {
                    error_at(type->token.pos, type->token.len, "type '" STR_FMT "' expects %d type argument(s), got %d", STR_ARG(name), expected, got);
                }
            }
        }
        for (Node *arg = type->ptr; arg; arg = arg->next) {
...
static void resolve_node(Resolver *r, Node *n) {
    ...
    case NODE_IDENT: {
        ...
        Node *decl = scope_lookup(r, name);
        if (!decl) {
            error_at(n->token.pos, n->token.len, "undefined identifier '" STR_FMT "'", STR_ARG(name));
        } else {
            n->decl = decl;
        }
        break;
...
static bool type_is_numeric(Node *t) { return type_is_integer(t) || type_is_float(t); }
...
static bool type_has_type_param(Node *t) {
    if (!t)
        return false;
    if (t->kind == NODE_TYPE_NAMED && t->decl && t->decl->kind == NODE_TYPE_PARAM) {
        return true;
    }
    if (t->kind == NODE_TYPE_PTR || t->kind == NODE_TYPE_SLICE || t->kind == NODE_TYPE_ARRAY) {
...
static Node *substitute_type(Arena *a, Node *type, Node *type_params, Node *type_args) {
    if (!type)
        return NULL;
    switch (type->kind) {
    case NODE_TYPE_NAMED: {
...
static Node *lookup_field(TypeChecker *tc, Node *struct_type, Str8 field_name, Node **out_type) {
    if (!struct_type)
        return NULL;
    Node *decl = NULL;
        ...
        if (field->kind == NODE_STRUCT_FIELD) {
            ...
            if (str8_eq(fname, field_name)) {
                ...
                if (type_params && type_args) {
                    field_type = substitute_type(tc->arena, field_type, type_params, type_args);
                }
                if (out_type)
                    *out_type = field_type;
                return field;
...
static Node *typecheck_expr(TypeChecker *tc, Node *n) {
    ...
    case NODE_BINARY: {
        ...
        case TK_SHR_EQ: {
            ...
            Node *result =
                (n->op == TK_SHL || n->op == TK_SHR || n->op == TK_SHL_EQ || n->op == TK_SHR_EQ) ? left_type : common_numeric_type(left_type, right_type);
            n->type = result;
        ...
        case TK_GE: {
            if (type_is_numeric(left_type) && type_is_numeric(right_type)) {
                ...
                if (!types_equal(base_left, base_right) && base_left->kind != NODE_TYPE_VOID && base_right->kind != NODE_TYPE_VOID) {
        ...
        case TK_ASSIGN: {
            ...
            if (!types_assignable(left_type, right_type)) {
                type_error(n, "cannot assign '%s' to '%s'", type_name(right_type), type_name(left_type));
            }
            n->type = left_type;
            return left_type;
        }
    ...
    case NODE_CALL: {
        ...
        if (!func_decl || (func_decl->kind != NODE_FUNC && func_decl->kind != NODE_FOREIGN)) {
        ...
        if (type_params) {
            ...
            while (param && arg && param->kind == NODE_PARAM) {
                ...
                if (param->type && param->type->kind == NODE_TYPE_NAMED && param->type->decl && param->type->decl->kind == NODE_TYPE_PARAM) {
                    int idx = 0;
                ...
                } else if (param->type && param->type->kind == NODE_TYPE_PTR && param->type->ptr && param->type->ptr->kind == NODE_TYPE_NAMED &&
                           param->type->ptr->decl && param->type->ptr->decl->kind == NODE_TYPE_PARAM) {
                ...
                } else if (param->type && param->type->kind == NODE_TYPE_SLICE && param->type->ptr && param->type->ptr->kind == NODE_TYPE_NAMED &&
                           param->type->ptr->decl && param->type->ptr->decl->kind == NODE_TYPE_PARAM) {
        ...
        while (param && arg) {
            ...
            if (type_params && inferred_args) {
                expected_type = substitute_type(tc->arena, param->type, type_params, inferred_args);
            }
            if (!types_assignable(expected_type, arg_type)) {
                type_error(arg, "argument %d: cannot convert '%s' to '%s'", arg_num, type_name(arg_type), type_name(expected_type));
            }
            param = param->next;
            arg = arg->next;
        }
        ...
        if (type_params && inferred_args && return_type) {
            return_type = substitute_type(tc->arena, return_type, type_params, inferred_args);
        }
        n->type = return_type;
        return return_type;
    }
    case NODE_INDEX: {
        ...
        if (!type_is_array(left_type) && !type_is_slice(left_type) && !type_is_pointer(left_type)) {
    ...
    case NODE_FIELD: {
        ...
        if (!left_type || (left_type->kind != NODE_TYPE_NAMED && left_type->kind != NODE_TYPE_GENERIC)) {
        ...
        if (!field) {
            type_error(n, "type has no field '%.*s'", (int)field_name.len, field_name.data);
            return NULL;
        }
        n->decl = field;
        n->type = field_type;
    ...
    case NODE_CAST: {
        ...
        if (type_is_integer(src_type) && type_is_pointer(dst_type))
            valid = true;
        if (!valid) {
            type_error(n, "invalid cast from '%s' to '%s'", type_name(src_type), type_name(dst_type));
        }
        return dst_type;
    }
    case NODE_SIZEOF:
    ...
    case NODE_MATCH: {
        ...
        for (Node *arm = n->ptr; arm; arm = arm->next) {
            ...
            if (pattern_type && match_type && !types_equal(pattern_type, match_type)) {
            ...
            if (!result_type) {
                ...
                if (types_equal(result_type, arm_result_type)) {
                    ...
                    result_type = arm_result_type;
                } else if (type_is_numeric(result_type) && type_is_numeric(arm_result_type)) {
                    result_type = common_numeric_type(result_type, arm_result_type);
                } else {
                    type_error(arm->right, "match arm %d has incompatible type '%s', expected '%s'", arm_num, type_name(arm_result_type),
        ...
        for (Node *init = n->ptr; init; init = init->next) {
            ...
            for (Node *prev = n->ptr; prev != init; prev = prev->next) {
                if (str8_eq(tok_text(prev->token), field_name)) {
                    type_error(init, "duplicate field '%.*s' in %s literal", (int)field_name.len, field_name.data, is_union ? "union" : "struct");
                    break;
                }
            }
            Node *field_type = NULL;
            ...
            if (!field) {
                type_error(init, "%s has no field '%.*s'", is_union ? "union" : "struct", (int)field_name.len, field_name.data);
                continue;
            }
            Node *init_type = typecheck_expr(tc, init->left);
            if (!types_assignable(field_type, init_type)) {
                type_error(init, "cannot assign '%s' to field '%.*s' of type '%s'", type_name(init_type), (int)field_name.len, field_name.data,
                           type_name(field_type));
            }
            init->decl = field;
        }
        if (is_union && field_count != 1) {
            type_error(n, "union literal must initialize exactly one field, got %d", field_count);
        }
        if (!is_union) {
            for (Node *field = decl->ptr; field; field = field->next) {
                if (field->kind != NODE_STRUCT_FIELD)
                ...
                if (!found) {
                    type_error(n, "missing field '%.*s' in struct literal", (int)field_name.len, field_name.data);
                }
            }
        }
        return struct_type;
...
static void typecheck_stmt(TypeChecker *tc, Node *n) {
    ...
    case NODE_RETURN: {
        ...
        if (n->left) {
            ...
            if (expected && !types_assignable(expected, actual)) {
                type_error(n, "return type mismatch: expected '%s', got '%s'", type_name(expected), type_name(actual));
            }
        } else {
    ...
    case NODE_CONTINUE:
        if (tc->loop_depth == 0) {
            type_error(n, "%s outside of loop", n->kind == NODE_BREAK ? "break" : "continue");
        }
        break;
    case NODE_DEFER:
        typecheck_expr(tc, n->left);
        ...
        if (n->left) {
            ...
            if (n->left->kind == NODE_IF && !n->type) {
                ...
                if (then_t && else_t && !types_equal(then_t, else_t)) {
                    type_error(n, "if expression branches have incompatible types: '%s' and '%s'", then_t ? type_name(then_t) : "?",
                               else_t ? type_name(else_t) : "?");
                }
            }
            if (n->type) {
                if (!types_assignable(n->type, init_type)) {
                    type_error(n, "cannot initialize '%s' with '%s'", type_name(n->type), type_name(init_type));
                }
            } else {
                n->type = init_type;
            }
            if (n->ptr && init_type && init_type->kind == NODE_TYPE_TUPLE) {
                ...
                if (name_count != type_count) {
                    type_error(n, "assignment count mismatch: %d variables but %d values", name_count, type_count);
                }
    ...
    case NODE_FUNC: {
        ...
        if (n->type && n->type->kind != NODE_TYPE_VOID) {
            if (!definitely_returns(n->left)) {
                type_error(n, "function '%.*s' must return a value on all paths", (int)tok_text(n->token).len, tok_text(n->token).data);
            }
        }
...
static LLVMTypeRef cg_type_subst(Codegen *cg, Node *type, Node *params, Node *args);
...
static LLVMTypeRef cg_type_subst(Codegen *cg, Node *type, Node *params, Node *args) {
    ...
    if (type->kind == NODE_TYPE_NAMED && type->decl && type->decl->kind == NODE_TYPE_PARAM) {
        Node *p = params;
        Node *a = args;
        while (p && a) {
            if (p == type->decl) {
...
static LLVMTypeRef cg_type(Codegen *cg, Node *type) {
    ...
    case NODE_TYPE_GENERIC: {
        ...
        if (decl->kind == NODE_STRUCT || decl->kind == NODE_UNION) {
            ...
            char name_buf[256];
            int pos = snprintf(name_buf, sizeof(name_buf), "%.*s", (int)base_name.len, base_name.data);
            for (Node *arg = type->ptr; arg && pos < 250; arg = arg->next) {
                if (arg->token.len > 0) {
                    Str8 arg_name = tok_text(arg->token);
                    pos += snprintf(name_buf + pos, sizeof(name_buf) - pos, "_%.*s", (int)arg_name.len, arg_name.data);
                }
            }
            LLVMTypeRef struct_type = LLVMStructCreateNamed(cg->ctx, name_buf);
...
static LLVMValueRef cg_lvalue(Codegen *cg, Node *n) {
    ...
    case NODE_FIELD: {
        ...
        if (decl_type->kind == NODE_TYPE_NAMED || decl_type->kind == NODE_TYPE_GENERIC) {
        ...
        if (n->left->type && n->left->type->kind == NODE_TYPE_PTR) {
            ...
            struct_ptr = LLVMBuildLoad2(cg->builder, ptr_val_type, struct_ptr, "ptr.val");
        }
        ...
        indices[1] = LLVMConstInt(LLVMInt32TypeInContext(cg->ctx), field_idx, false);
...
static LLVMValueRef cg_convert(Codegen *cg, LLVMValueRef val, Node *src_type, Node *dst_type) {
    ...
    if (type_is_integer(src_type) && type_is_integer(dst_type)) {
        ...
        if (dst_bits > src_bits) {
            return type_is_signed(src_type) ? LLVMBuildSExt(cg->builder, val, llvm_dst, "") : LLVMBuildZExt(cg->builder, val, llvm_dst, "");
    ...
    if (type_is_float(src_type) && type_is_float(dst_type)) {
        ...
        } else if (dst_type->kind == NODE_TYPE_F32 && src_type->kind == NODE_TYPE_F64) {
            return LLVMBuildFPTrunc(cg->builder, val, llvm_dst, "");
        }
        return val;
    }
    if (type_is_integer(src_type) && type_is_float(dst_type)) {
        return type_is_signed(src_type) ? LLVMBuildSIToFP(cg->builder, val, llvm_dst, "") : LLVMBuildUIToFP(cg->builder, val, llvm_dst, "");
    }
    if (type_is_float(src_type) && type_is_integer(dst_type)) {
        return type_is_signed(dst_type) ? LLVMBuildFPToSI(cg->builder, val, llvm_dst, "") : LLVMBuildFPToUI(cg->builder, val, llvm_dst, "");
    }
    return val;
}
static LLVMValueRef cg_expr(Codegen *cg, Node *n) {
    ...
    case NODE_BINARY: {
        ...
        bool is_comparison = (n->op == TK_EQ || n->op == TK_NE || n->op == TK_LT || n->op == TK_LE || n->op == TK_GT || n->op == TK_GE);
        ...
        switch (n->op) {
        case TK_PLUS:
            return is_float ? LLVMBuildFAdd(cg->builder, lhs, rhs, "") : LLVMBuildAdd(cg->builder, lhs, rhs, "");
        case TK_MINUS:
            return is_float ? LLVMBuildFSub(cg->builder, lhs, rhs, "") : LLVMBuildSub(cg->builder, lhs, rhs, "");
        case TK_STAR:
            return is_float ? LLVMBuildFMul(cg->builder, lhs, rhs, "") : LLVMBuildMul(cg->builder, lhs, rhs, "");
        case TK_SLASH:
            if (is_float)
                return LLVMBuildFDiv(cg->builder, lhs, rhs, "");
            return is_signed ? LLVMBuildSDiv(cg->builder, lhs, rhs, "") : LLVMBuildUDiv(cg->builder, lhs, rhs, "");
        case TK_PERCENT:
            if (is_float)
                return LLVMBuildFRem(cg->builder, lhs, rhs, "");
            return is_signed ? LLVMBuildSRem(cg->builder, lhs, rhs, "") : LLVMBuildURem(cg->builder, lhs, rhs, "");
        case TK_AMP:
        ...
        case TK_SHR:
            return is_signed ? LLVMBuildAShr(cg->builder, lhs, rhs, "") : LLVMBuildLShr(cg->builder, lhs, rhs, "");
        case TK_EQ:
            return is_float ? LLVMBuildFCmp(cg->builder, LLVMRealOEQ, lhs, rhs, "") : LLVMBuildICmp(cg->builder, LLVMIntEQ, lhs, rhs, "");
        case TK_NE:
            return is_float ? LLVMBuildFCmp(cg->builder, LLVMRealONE, lhs, rhs, "") : LLVMBuildICmp(cg->builder, LLVMIntNE, lhs, rhs, "");
        case TK_LT:
            if (is_float)
                return LLVMBuildFCmp(cg->builder, LLVMRealOLT, lhs, rhs, "");
            return is_signed ? LLVMBuildICmp(cg->builder, LLVMIntSLT, lhs, rhs, "") : LLVMBuildICmp(cg->builder, LLVMIntULT, lhs, rhs, "");
        case TK_LE:
            if (is_float)
                return LLVMBuildFCmp(cg->builder, LLVMRealOLE, lhs, rhs, "");
            return is_signed ? LLVMBuildICmp(cg->builder, LLVMIntSLE, lhs, rhs, "") : LLVMBuildICmp(cg->builder, LLVMIntULE, lhs, rhs, "");
        case TK_GT:
            if (is_float)
                return LLVMBuildFCmp(cg->builder, LLVMRealOGT, lhs, rhs, "");
            return is_signed ? LLVMBuildICmp(cg->builder, LLVMIntSGT, lhs, rhs, "") : LLVMBuildICmp(cg->builder, LLVMIntUGT, lhs, rhs, "");
        case TK_GE:
            if (is_float)
                return LLVMBuildFCmp(cg->builder, LLVMRealOGE, lhs, rhs, "");
            return is_signed ? LLVMBuildICmp(cg->builder, LLVMIntSGE, lhs, rhs, "") : LLVMBuildICmp(cg->builder, LLVMIntUGE, lhs, rhs, "");
        case TK_AND:
            return LLVMBuildAnd(cg->builder, lhs, rhs, "");
        case TK_OR:
            return LLVMBuildOr(cg->builder, lhs, rhs, "");
    ...
    case NODE_UNARY: {
        ...
        case TK_BANG:
            return LLVMBuildXor(cg->builder, operand, LLVMConstInt(LLVMInt1TypeInContext(cg->ctx), 1, false), "");
        case TK_TILDE:
    ...
    case NODE_CALL: {
        ...
        LLVMTypeRef func_type = LLVMFunctionType(ret_type, param_types, param_count, is_vararg);
    ...
    case NODE_IF: {
        ...
        if (!cond)
            return NULL;
        LLVMBasicBlockRef then_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "then");
        LLVMBasicBlockRef else_bb = n->ptr ? LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "else") : NULL;
        LLVMBasicBlockRef merge_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "ifcont");
    ...
    case NODE_SIZEOF: {
        ...
        return LLVMBuildIntCast2(cg->builder, size, LLVMInt64TypeInContext(cg->ctx), false, "");
    }
    case NODE_ALIGNOF: {
        LLVMTypeRef t = cg_type(cg, n->left);
        LLVMValueRef align = LLVMAlignOf(t);
        return LLVMBuildIntCast2(cg->builder, align, LLVMInt64TypeInContext(cg->ctx), false, "");
    }
    case NODE_INDEX: {
        LLVMValueRef base = cg_expr(cg, n->left);
        LLVMValueRef index = cg_expr(cg, n->right);
        ...
        if (base_type->kind == NODE_TYPE_ARRAY) {
            ...
            LLVMTypeRef arr_type = cg_type(cg, base_type);
            LLVMValueRef elem_ptr = LLVMBuildGEP2(cg->builder, arr_type, array_ptr, indices, 2, "");
            return LLVMBuildLoad2(cg->builder, elem_type, elem_ptr, "");
        } else if (base_type->kind == NODE_TYPE_PTR) {
            LLVMValueRef elem_ptr = LLVMBuildGEP2(cg->builder, elem_type, base, &index, 1, "");
            return LLVMBuildLoad2(cg->builder, elem_type, elem_ptr, "");
        } else if (base_type->kind == NODE_TYPE_SLICE) {
            LLVMTypeRef slice_type = cg_type(cg, base_type);
            LLVMValueRef data_ptr = LLVMBuildExtractValue(cg->builder, base, 0, "slice.data");
            LLVMValueRef elem_ptr = LLVMBuildGEP2(cg->builder, elem_type, data_ptr, &index, 1, "");
            (void)slice_type;
            return LLVMBuildLoad2(cg->builder, elem_type, elem_ptr, "");
        }
        return NULL;
    ...
    case NODE_FIELD: {
        ...
        if (search_type->kind == NODE_TYPE_NAMED || search_type->kind == NODE_TYPE_GENERIC) {
            struct_decl = search_type->decl;
        }
        if (!struct_decl || (struct_decl->kind != NODE_STRUCT && struct_decl->kind != NODE_UNION)) {
            return NULL;
        }
        Str8 field_name = tok_text(n->token);
        int field_idx = 0;
        ...
        if (n->left->kind == NODE_IDENT && n->left->decl) {
            ...
            LLVMValueRef tmp = LLVMBuildAlloca(cg->builder, obj_llvm_type, "struct.tmp");
        ...
        indices[0] = LLVMConstInt(LLVMInt32TypeInContext(cg->ctx), 0, false);
        indices[1] = LLVMConstInt(LLVMInt32TypeInContext(cg->ctx), field_idx, false);
        LLVMValueRef field_ptr = LLVMBuildGEP2(cg->builder, struct_llvm_type, struct_ptr, indices, 2, "");
    ...
    case NODE_ARRAY_LIT: {
        ...
        bool all_const = true;
        for (Node *elem = n->ptr; elem; elem = elem->next) {
            if (elem->kind != NODE_INT && elem->kind != NODE_FLOAT && elem->kind != NODE_BOOL && elem->kind != NODE_CHAR) {
                all_const = false;
                break;
            }
        }
        if (all_const) {
            ...
            for (Node *elem = n->ptr; elem; elem = elem->next) {
                ...
                LLVMValueRef elem_ptr = LLVMBuildGEP2(cg->builder, arr_type, arr, indices, 2, "");
    ...
    case NODE_STRUCT_LIT: {
        ...
        if (!struct_type)
            return NULL;
        Node *struct_decl = NULL;
        if (struct_type->kind == NODE_TYPE_NAMED || struct_type->kind == NODE_TYPE_GENERIC) {
            struct_decl = struct_type->decl;
        }
        if (!struct_decl)
            return NULL;
        LLVMTypeRef llvm_struct_type = cg_type(cg, struct_type);
        LLVMValueRef struct_ptr = LLVMBuildAlloca(cg->builder, llvm_struct_type, "struct.lit");
        ...
        for (Node *init = n->ptr; init; init = init->next) {
            ...
            indices[0] = LLVMConstInt(LLVMInt32TypeInContext(cg->ctx), 0, false);
            indices[1] = LLVMConstInt(LLVMInt32TypeInContext(cg->ctx), field_idx, false);
            LLVMValueRef field_ptr = LLVMBuildGEP2(cg->builder, llvm_struct_type, struct_ptr, indices, 2, "");
    ...
    case NODE_FOR: {
        ...
        if (n->ptr && n->ptr->kind == NODE_VAR) {
            ...
            if (range && range->kind == NODE_RANGE) {
                ...
                LLVMBasicBlockRef cond_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "for.cond");
                LLVMBasicBlockRef body_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "for.body");
                LLVMBasicBlockRef incr_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "for.incr");
                LLVMBasicBlockRef end_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "for.end");
                ...
                LLVMValueRef iter_val = LLVMBuildLoad2(cg->builder, iter_type, iter_ptr, "i.val");
                LLVMValueRef cond;
                ...
                LLVMValueRef curr = LLVMBuildLoad2(cg->builder, iter_type, iter_ptr, "");
                LLVMValueRef next = LLVMBuildAdd(cg->builder, curr, LLVMConstInt(iter_type, 1, false), "i.next");
        ...
        if (n->left) {
            LLVMBasicBlockRef cond_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "while.cond");
            LLVMBasicBlockRef body_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "while.body");
            LLVMBasicBlockRef end_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "while.end");
            LLVMBasicBlockRef saved_break = cg->break_target;
            LLVMBasicBlockRef saved_continue = cg->continue_target;
            cg->break_target = end_bb;
    ...
    case NODE_MATCH: {
        ...
        LLVMBasicBlockRef merge_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "match.end");
        ...
        while (arm) {
            ...
            if (is_wildcard) {
                default_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "match.default");
                ...
                LLVMValueRef cond = LLVMBuildICmp(cg->builder, LLVMIntEQ, scrutinee, pattern_val, "");
                LLVMBasicBlockRef then_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "match.arm");
                LLVMBasicBlockRef else_bb = LLVMAppendBasicBlockInContext(cg->ctx, cg->current_func, "match.next");
        ...
        if (!default_bb && !LLVMGetBasicBlockTerminator(LLVMGetInsertBlock(cg->builder))) {
            LLVMBuildBr(cg->builder, merge_bb);
        }
        LLVMPositionBuilderAtEnd(cg->builder, merge_bb);
    ...
    case NODE_SLICE: {
        ...
        if (base_type->kind == NODE_TYPE_ARRAY) {
            ...
            indices[1] = lo;
            LLVMTypeRef arr_type = cg_type(cg, base_type);
            data_ptr = LLVMBuildGEP2(cg->builder, arr_type, array_ptr, indices, 2, "");
            if (!hi) {
                i64 arr_len = 0;
...
static void cg_stmt(Codegen *cg, Node *n) {
    ...
    case NODE_VAR: {
        if (n->ptr && n->left && n->left->type && n->left->type->kind == NODE_TYPE_TUPLE) {
            ...
            for (Node *name = n->ptr; name; name = name->next, i++) {
                ...
                char name_buf[256];
                snprintf(name_buf, sizeof(name_buf), "%.*s", (int)var_name.len, var_name.data);
...
static void cg_decl(Codegen *cg, Node *n) {
    ...
    case NODE_FUNC: {
        ...
        LLVMBasicBlockRef entry = LLVMAppendBasicBlockInContext(cg->ctx, func, "entry");
...
static void codegen(Arena *arena, Node **decls, usize count, const char *bin_name, const char *dump_ir_path) {
    Codegen cg = {0};
    cg.arena = arena;
    ...
    LLVMTargetMachineRef machine =
        LLVMCreateTargetMachine(target, triple, host_cpu, host_features, LLVMCodeGenLevelDefault, LLVMRelocPIC, LLVMCodeModelDefault);
    ...
    if (dump_ir_path) {
        ...
        if (LLVMPrintModuleToFile(cg.mod, dump_ir_path, &err_msg) != 0) {
            fatal("failed to write IR to %s: %s\n", dump_ir_path, err_msg ? err_msg : "unknown");
            if (err_msg) {
                LLVMDisposeMessage(err_msg);
            }
        }
    ...
    if (LLVMTargetMachineEmitToFile(machine, cg.mod, obj_name, LLVMObjectFile, &err_msg)) {
